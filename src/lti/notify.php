  ...
  if ($is_saved) {
    if ($resource_link->hasOutcomesService()) {
      ...
      if ($resource_link->doOutcomesService(LTI_Resource_Link::EXT_WRITE, $outcome)) {
        ...
        error_log("Failed to pass outcome of {$score} for {$result_id}. Variables set: Resource Link: {$resource_link}, Consumer: {$consumer}, Outcome: " . print_r($outcome, TRUE) . " POST: " . print_r($_POST, TRUE));
      }
    }
  } else {
